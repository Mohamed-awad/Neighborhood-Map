...
var list_locations = ko.observableArray([
	'Park Ave Penthouse',
	'Chelsea Loft',
...
var searchWithinPolygon = function () {
...
var initMap = function() {
	...
	document.getElementById('location1').addEventListener(
		'click', function (){
			showInfoWindow('Park Ave Penthouse');
	});

	document.getElementById('location2').addEventListener(
		'click', function (){
			showInfoWindow('Chelsea Loft');
	});
	document.getElementById('location3').addEventListener(
		'click', function (){
			showInfoWindow('Union Square Open Floor Plan');
	});
	document.getElementById('location4').addEventListener(
		'click', function (){
			showInfoWindow('East Village Hip Studio');
	});
	document.getElementById('location5').addEventListener(
		'click', function (){
			showInfoWindow('TriBeCa Artsy Bachelor Pad');
	});
	document.getElementById('location6').addEventListener(
		'click', function (){
		...
		geocoder.geocode(
			{
				...
				if (status == google.maps.GeocoderStatus.OK) {
					...
					window.alert('we could not find that '+
						'location - try entering a more' + 
						' specific place.');
				}
			}
		);
				...
				if (duration <= maxDuration) {
					...
					var infowindow = new google.maps.InfoWindow({
						content: durationText + 'away, ' + distanceText +
								'<div><input type=\"button\" value=\"View Route\" onclick =\"displayDirections(&quot;' + origins[i] +
                    			'&quot;);\"></input></div>'
					});
...
var hideMarkerAndItemLocation = function (unmatched_location, matched_location) {
	...
	for (i = 0; i < matched_location.length; i++) {
